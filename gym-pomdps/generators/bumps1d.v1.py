...
def sfmt(s):
    return f'cart{s.pos.cart.value}_theta{s.pos.angle.value}_lb{s.pos.sbump.value}_rb{s.pos.bbump.value}_t{s.pos.touch.value}'
...
def main():
    ...
    both_bumps_touched = ['S', 'SB', 'B', '0']
    touch_space = indextools.DomainSpace(both_bumps_touched)
    ...
    start_states = [
        s
        for s in all_states if s.pos.angle.value == 'Z' and s.pos.touch.value == '0'
    ]
                ...
                if check_state(s):
                    ...
                    if (s.pos.cart.value >= min_cart_pos + 1):
                        ...
                        if s.pos.cart.value == s.pos.sbump.value + 1:
                            ...
                            if (s1.pos.touch.value in ['0']):
                                s1.pos.touch.value = 'S'
                            elif s1.pos.touch.value in ['B']:
                                s1.pos.touch.value = 'SB'
                        ...
                        # big bump on the left
                            ...
                            if (s1.pos.touch.value in ['0']):
                                s1.pos.touch.value = 'B'
                            elif s1.pos.touch.value in ['S']:
                                s1.pos.touch.value = 'SB'
    ...
    for a in action_space.elems:
        if a.value == 'RH':
            for s in state_space.elems:
                if check_state(s):
                    if s.pos.cart.value == s.pos.bbump.value and ('L' in s.pos.angle.value) and (s.pos.touch.value in ['SB']):
                        print(f'R: {afmt(a)}: {sfmt(s)}: *: * 1.0')

                    if s.pos.cart.value == s.pos.bbump.value - 1  and (s.pos.angle.value == 'Z') and (s.pos.touch.value in ['SB']):
                        print(f'R: {afmt(a)}: {sfmt(s)}: *: * 1.0')
